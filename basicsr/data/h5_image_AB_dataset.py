...
class H5ImageDataset(data.Dataset):
    ...
    def __init__(self, opt, return_voxel=True, return_frame=True, return_gt_frame=True,
        ...
        self.opt = opt
        if "train" in self.opt["name"]:
            file_folder_pathA = opt['datarootA']
            file_folder_pathB = opt['datarootB']
            if os.path.isdir(file_folder_pathA):
                self.h5_file_path = [os.path.join(file_folder_pathA, s) for s in os.listdir(file_folder_pathA)]
            if os.path.isdir(file_folder_pathB):
                self.h5_file_path.extend([os.path.join(file_folder_pathB, s) for s in os.listdir(file_folder_pathB)])
        else:
            file_folder_path = opt['dataroot']
            if os.path.isdir(file_folder_path):
