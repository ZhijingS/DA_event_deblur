...
class H5ImageDataset(data.Dataset):
    ...
    def get_frame(self, index):
        ...
        return  self.h5_file['images']['image{:09d}'.format(index)][:] #HS_ERGB是blur_images其他都是images
    ...
    def get_gt_frame(self, index):
        ...
        if self.h5_file is None:
            self.h5_file = h5py.File(self.data_path, 'r')
        return self.h5_file['sharp_images']['image{:09d}'.format(index)][:] #HS_ERGB是gt_images其他都是sharp_images
    ...
    def get_voxel(self, index):
        ...
        if self.h5_file is None:
            self.h5_file = h5py.File(self.data_path, 'r')
        return self.h5_file['voxels']['voxel{:09d}'.format(index)][:] #HS_ERGB是event_voxels其他都是voxels
    ...
    def __init__(self, opt, data_path, return_voxel=True, return_frame=True, return_gt_frame=True,
        ...
        with h5py.File(self.data_path, 'r') as file:
            ...
            self.dataset_len = len(file['images'].keys()) #HS_ERGB是"blur_images"其他都是"images"
    ...
    def transform_frame(self, frame, seed, transpose_to_CHW=False):
        ...
        if self.return_format == "torch":
            if transpose_to_CHW:
                frame = torch.from_numpy(frame.copy().transpose(2, 0, 1)).float() / 255  # H,W,C -> C,H,W

            else:
                frame = torch.from_numpy(frame.copy()).float() / 255 # 0-1
            if self.transform:
                random.seed(seed)
    ...
    def transform_voxel(self, voxel, seed, transpose_to_CHW):
        ...
        if self.return_format == "torch":
            if transpose_to_CHW:
                voxel = torch.from_numpy(voxel.copy().transpose(2, 0, 1)).float()# H,W,C -> C,H,W

            else:
                if self.norm_voxel:
                    voxel = torch.from_numpy(voxel.copy()).float() / abs(max(voxel.min(), voxel.max(), key=abs))  # -1 ~ 1
                else:
                    voxel = torch.from_numpy(voxel.copy()).float()
