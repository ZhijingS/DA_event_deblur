...
class TestImageEventRestorationModel(BaseModel):
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.opt['datasets']['train'].get('use_mask'):
            ...
            preds = self.net_g(self.lq, event = self.voxel, mask = self.mask)

        else:
            preds = self.net_g(self.lq, event = self.voxel)
    ...
    def plot_magnitude(self):
        for i in range(len(self.output)):
            pic_name = str(i)+'.png'
            save_img_path = osp.join(self.opt['path']['results_root'], self.seq_name, pic_name)
            dir_name = os.path.abspath(os.path.dirname(save_img_path))
            os.makedirs(dir_name, exist_ok=True)
            plt.imsave(save_img_path, self.output[i].squeeze(), cmap='gray')
