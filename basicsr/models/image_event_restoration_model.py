...
class ImageEventRestorationModel(BaseModel):
    ...
    def optimize_parameters(self, current_iter):
        ...
        self.output = preds[-1] # 如果只用输出图片作为loss计算，不需要储存list
        ...
        if self.cri_pix:
            ...
            elif self.pixel_type == 'PSNR_tri_Loss' or 'PSNR_trils_Loss':
                for pred in preds:
        ...
        with torch.no_grad():
            ...
            while i < n:
                ...
                if self.opt['datasets']['val'].get('use_mask'):
                    ...
                    pred = self.net_g(x = self.lq[i:j, :, :, :], event = self.voxel[i:j, :, :, :])
                    import pdb;pdb.set_trace()  # mini batch all in 
